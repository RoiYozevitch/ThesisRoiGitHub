...
public class SirfMLCsvWriter {
    ...
    private static String header2 = /*" *//*SVs number, PRN, GPS Software Time, Azimuth,*/ "Elevation,"/*+  "State,"  */+ "Is Carrier Lock, residuals, C/No Max, C/No Min, " +
            "MaxCno2Second, ExtremeMaxSnrValue, ExtremeMinSnrValue, Delta Carrier Freq, Delta Carrier phase, Clock Bias, Clock Drift, Delta Ionospheric Delay," +
    ...
    public static void printToFileSpecificValues(List<SirfPeriodicMeasurement> measurements, String path) throws IOException {
        BufferedWriter writer = new BufferedWriter(new FileWriter(path + ".csv"));
...
// ExtremeMinSnrValue,Delta Carrier Freq, Delta Carrier phase, Clock Bias, Clock Drift, Delta Ionospheric Delay, deltaCorrected abs PR, 2nd delta,PS Residual, Los Value" + newLine;
    ...
    private static String toCsvRestrictedString(SirfPeriodicMeasurement meas)
    {
        ...
        for (Integer PRN : meas.getSatellites().keySet()){
            ...
            if(sv.getElevation()>=5&& sv.getMaxCn0()>0) {
